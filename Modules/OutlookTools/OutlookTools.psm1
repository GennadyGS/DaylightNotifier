...
#========================================================================
	...
	PROCESS { 
             ...
             if ($CheckDuplicates -and
                ($calendarFolder.Items.Find("[Subject]=`"$Subject`" And [Start]=`"$($MeetingStart.ToString("yyyy-MM-dd HH:mm"))`""))) {
                    return
             }
             $newCalenderItem = $calendarFolder.Items.Add(1)
	         ...
	         $newCalenderItem.Sensitivity = $Sensitivity
	         $newCalenderItem.Categories = $Categories -join ","
    ...
    Function Get-OutlookCalendarFolder {
        ...
        $calendarRootFolder = $rootFolder.Folders.Item("Calendar")
        if (!($calendarRootFolder)) {
            throw "Calendar root folder not found"
        }
        if (!$calendarName) {
            return $calendarRootFolder
        }
        $calendarFolder = $calendarRootFolder.Folders.Item($calendarName)
        if (!($calendarFolder)) {
            throw "Calendar with name $calendarName not found"
        }
